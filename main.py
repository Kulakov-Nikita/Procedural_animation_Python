...
chain: Chain = Chain(position=torch.tensor((-10, -50, 200), dtype=torch.float32), radius=20, length=10, tile_size=resolution)

record_video(path='videos/test.avi', resolution=(resolution, resolution), fps=30, num_frames=90,
             renderer=renderer, objects=chain.chain_links, chain=chain)
